...
- **Settings: provider presets for summaries.** Gateway settings use a Provider select (Vercel AI Gateway, OpenAI, Anthropic, OpenRouter, Gemini, Groq, Ollama) that fills the OpenAI-compatible base URL and a sensible model default; **Custom…** reveals a base URL field for anything else. ZDR still only applies to Vercel.
...
- **Settings: surface OpenAI-compatible gateways.** Panel retitled **AI Gateway**; subtitle and Base URL hint explain that any Chat Completions endpoint works (Gemini example included). ZDR toggle is labeled Vercel-only and hides when Base URL isn’t `ai-gateway.vercel.sh`. Key hints and summary API errors no longer assume Vercel.
